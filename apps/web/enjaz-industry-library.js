...
function installNav(){
 ...
 const button=document.createElement('button');button.type='button';button.dataset.enjazLibrary='1';button.innerHTML='<i>▦</i><span>مكتبة الموظفين</span>';button.addEventListener('click',()=>{document.querySelectorAll('.enjaz-nav button').forEach(x=>x.classList.remove('active'));button.classList.add('active');renderLibrary();});
 const employees=nav.querySelector('[data-nav="employees"]');employees?.insertAdjacentElement('afterend',button);
}
async function renderLibrary(){
 const content=document.getElementById('content');if(!content)return;
 content.innerHTML='<section class="pagehead"><div><div class="eyebrow">READY WORKFORCE CATALOG</div><h1>مكتبة الموظفين الجاهزين</h1><p>قوالب نظامية جاهزة حسب القطاع. عند التفعيل تُنشأ موظفون رقميون حقيقيون في مساحة عملك مع أهداف ومعرفة وصلاحيات مضبوطة.</p></div></section><div class="panel"><div class="e360-loading">جارٍ تحميل الكتالوج…</div></div>';
 try{
  const result=await apiClient.industryPacks(workspace(),token());const packs=Array.isArray(result.data)?result.data:[];
  content.innerHTML=`<section class="pagehead"><div><div class="eyebrow">READY WORKFORCE CATALOG</div><h1>${navLabel}</h1><p>اختر قطاعًا لتجهيز قوة عمل رقمية جاهزة. الكتالوج نظامي، أما الموظفون الذين يتم إنشاؤهم فهم بيانات حقيقية لمساحة العمل.</p></div><button type="button" class="ghost" data-library-refresh>تحديث</button></section><div class="employee-grid">${packs.map(pack=>`<article class="employee-card"><div class="employee-card-head"><span class="avatar avatar-lg">${esc(pack.label?.charAt(0)||'م')}</span><div><h3>${esc(pack.label)}</h3><p>${esc(pack.id)}</p></div><span class="dot-status"></span></div><div class="employee-tags"><span>${Number(pack.roles||0)} أدوار جاهزة</span><span>${Number(pack.departments||0)} أقسام</span></div><div class="employee-metrics"><div><b>${Number(pack.roles||0)}</b><small>موظفون</small></div><div><b>${Number(pack.departments||0)}</b><small>أقسام</small></div><div><b>جاهز</b><small>System Catalog</small></div></div><button type="button" class="primary" data-provision-pack="${esc(pack.id)}">تجهيز هذا القطاع</button></article>`).join('')||'<div class="empty"><h2>لا توجد حزم جاهزة</h2><p>تحقق من اتصال خدمة الكتالوج.</p></div>'}</div>`;
