...
async function renderLibrary(){
 ...
 try{
  ...
  content.innerHTML=`<section class="pagehead"><div><div class="eyebrow">READY WORKFORCE CATALOG</div><h1>${navLabel}</h1><p>قوة عمل جاهزة حسب القطاع: مطاعم، مستشفيات، فنادق، شركات، جهات حكومية، ومتاجر إلكترونية. لا تحتاج إلى بناء موظف من الصفر.</p></div><button type="button" class="ghost" data-library-refresh>تحديث</button></section><div class="employee-grid">${packs.map(pack=>`<article class="employee-card"><div class="employee-card-head"><span class="avatar avatar-lg">${esc(pack.label?.charAt(0)||'م')}</span><div><h3>${esc(pack.label)}</h3><p>${esc(pack.id)}</p></div><span class="dot-status"></span></div><div class="employee-tags"><span>${Number(pack.roles||0)} أدوار جاهزة</span><span>${Number(pack.departments||0)} أقسام</span></div><div class="employee-metrics"><div><b>${Number(pack.roles||0)}</b><small>أدوار</small></div><div><b>${Number(pack.departments||0)}</b><small>أقسام</small></div><div><b>جاهز</b><small>System Catalog</small></div></div><button type="button" class="primary" data-provision-pack="${esc(pack.id)}">تجهيز هذا القطاع</button></article>`).join('')||'<div class="empty"><h2>لا توجد حزم جاهزة</h2><p>تحقق من اتصال خدمة الكتالوج.</p></div>'}</div>`;
