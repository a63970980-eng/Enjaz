...
const workspaceSetup=async profile=>{const safeProfile=profile&&typeof profile==='object'?profile:{};const el=mount(`<div class="auth-shell"><div class="auth-card onboarding-card"><div class="auth-brand"><span class="brand-mark"><i></i><i></i><i></i></span><div><strong>إنجاز</strong><small>مساحة العمل الأولى</small></div></div><div class="auth-copy"><div class="eyebrow">01 / WORKSPACE</div><h1>لنجهّز شركتك</h1><p>أنشئ المؤسسة ومساحة العمل الأولى ثم ادخل مباشرة إلى مركز القيادة.</p></div><form id="workspace-form"><label>اسم المؤسسة<input name="organizationName" required placeholder="مثال: شركة إنجاز التقنية"></label><label>اسم مساحة العمل<input name="workspaceName" required placeholder="مثال: العمليات اليومية"></label><button class="primary auth-submit" type="submit">إنشاء مساحة العمل ←</button></form><div class="auth-secure">سيتم إنشاء المؤسسة والعضوية الأولى في عملية آمنة.</div></div></div>`);el.querySelector('#workspace-form')?.addEventListener('submit',async e=>{e.preventDefault();const b=new FormData(e.currentTarget),btn=e.currentTarget.querySelector('button');btn.disabled=true;btn.textContent='جارٍ تجهيز الشركة…';try{const data=await authClient.bootstrap(apiBase,{name:safeProfile.identity?.name||safeProfile.user?.name||'',organizationName:b.get('organizationName'),workspaceName:b.get('workspaceName')});const workspaces=Array.isArray(data?.workspaces)?data.workspaces:[];const ws=workspaces[0]||data?.workspace;if(!ws)throw new Error('لم يتم إنشاء مساحة العمل.');localStorage.setItem('ENJAZ_WORKSPACE_ID',ws.id);sessionStorage.setItem('ENJAZ_WORKSPACES',JSON.stringify(workspaces.length?workspaces:[{...ws,role:data.role||ws.workspace_role||'owner'}]));window.ENJAZ_WORKSPACE_ID=ws.id;window.ENJAZ_ACCESS_TOKEN=authClient.token();setAuthState('authenticated');clearGate();window.location.reload()}catch(err){btn.disabled=false;btn.textContent='إنشاء مساحة العمل ←';const old=el.querySelector('.auth-error');old?.remove();el.querySelector('.auth-card')?.insertAdjacentHTML('beforeend',`<div class="auth-error" role="alert">${esc(err?.message||'تعذر إنشاء مساحة العمل.')}</div>`)}})};

async function recover(){try{const profile=await authClient.me(apiBase);if(!profile){authClient.signOut();setAuthState('public');window.location.href=location.origin+location.pathname;return}const memberships=Array.isArray(profile.workspaces)?profile.workspaces:[];if(!memberships.length){setAuthState('auth');await workspaceSetup(profile);return}const requested=q.get('workspaceId')||localStorage.getItem('ENJAZ_WORKSPACE_ID')||'';const selected=requested&&memberships.some(w=>String(w.id)===String(requested))?memberships.find(w=>String(w.id)===String(requested)):memberships[0];localStorage.setItem('ENJAZ_WORKSPACE_ID',selected.id);sessionStorage.setItem('ENJAZ_WORKSPACES',JSON.stringify(memberships));sessionStorage.setItem('ENJAZ_USER_PROFILE',JSON.stringify(profile));window.ENJAZ_WORKSPACE_ID=selected.id;window.ENJAZ_ACCESS_TOKEN=authClient.token();const enteredFromAuth=q.has('auth')||q.has('signup')||q.has('workspaceId');if(enteredFromAuth){const url=new URL(location.href);url.search='';history.replaceState({},'',url.toString())}setAuthState('authenticated');clearGate();window.dispatchEvent(new CustomEvent('enjaz:authenticated'));
// The explicit auth route is a standalone boot path in main.js. It does not
// import the workspace application, so after successful authentication we
// must restart the normal boot path instead of leaving the locked auth DOM
// removed over an otherwise empty document.
if(enteredFromAuth)window.location.reload();
