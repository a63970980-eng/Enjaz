...
const confirmation=({email,error='' }={})=>{const el=mount(`<div class="auth-shell"><div class="auth-card session-recovery confirmation-card"><div class="auth-brand"><span class="brand-mark">إ</span><div><strong>إنجاز</strong><small>ENJAZ · EMAIL VERIFICATION</small></div></div><div class="auth-copy"><div class="eyebrow">VERIFY YOUR EMAIL</div><h1>تحقق من بريدك الإلكتروني</h1><p>أرسلنا رابط تفعيل إلى <strong>${esc(email)}</strong>. بعد التفعيل عد إلى إنجاز وسجّل الدخول للمتابعة.</p></div>${error?`<div class="auth-error" role="alert">${esc(error)}</div>`:''}<div class="auth-recovery-actions"><button class="primary" id="resend-confirmation" type="button">إعادة إرسال رابط التفعيل</button><button class="ghost" id="back-login" type="button">العودة لتسجيل الدخول</button></div><div class="auth-secure">لن نطلب منك كلمة المرور عبر البريد. تحقق من عنوان المرسل قبل فتح الرابط.</div></div></div>`);el.querySelector('#resend-confirmation')?.addEventListener('click',async()=>{const btn=el.querySelector('#resend-confirmation');btn.disabled=true;btn.textContent='جارٍ الإرسال…';try{await authClient.resendConfirmation(email);btn.textContent='تم إرسال الرابط ✓';}catch(err){btn.disabled=false;btn.textContent='إعادة إرسال رابط التفعيل';el.querySelector('.auth-card')?.insertAdjacentHTML('beforeend',`<div class="auth-error" role="alert">${esc(err?.message||'تعذر إرسال رابط التفعيل.')}</div>`)} });el.querySelector('#back-login')?.addEventListener('click',()=>{q.delete('signup');history.replaceState({},'',`${location.pathname}?auth=1`);renderForm()});return el};
...
const workspaceSetup=async profile=>{
 ...
 el.querySelector('#workspace-form')?.addEventListener('submit',async e=>{
  ...
  try{
   ...
   try{await apiClient.provisionIndustryPack(workspace.id,authClient.token(),sector)}catch(error){provisioningError=error?.message||'تعذر تفعيل حزمة القطاع تلقائيًا.';console.warn('[ENJAZ_ONBOARDING_PROVISION]',error)}
  ...
  }catch(err){btn.disabled=false;btn.textContent='إنشاء المؤسسة وتفعيل القطاع ←';const old=el.querySelector('.auth-error');old?.remove();el.querySelector('.auth-card')?.insertAdjacentHTML('beforeend',`<div class="auth-error" role="alert">${esc(err?.message||'تعذر إنشاء مساحة العمل.')}</div>`)}
 });
};

const renderForm=({error='' }={})=>{const el=form({error});el.querySelector('#auth-switch')?.addEventListener('click',()=>goAuth(!isSignup()));el.querySelector('#auth-form')?.addEventListener('submit',async e=>{e.preventDefault();const b=new FormData(e.currentTarget),btn=e.currentTarget.querySelector('button');btn.disabled=true;btn.textContent=isSignup()?'جارٍ إنشاء الحساب…':'جارٍ تسجيل الدخول…';try{if(isSignup()){const result=await authClient.signUp(b.get('email'),b.get('password'),b.get('name'));if(result?.needsEmailConfirmation){confirmation({email:b.get('email')});return}}else await authClient.signIn(b.get('email'),b.get('password'));await recover()}catch(err){btn.disabled=false;btn.textContent=isSignup()?'إنشاء الحساب':'تسجيل الدخول';renderForm({error:err?.message||'تعذر إكمال العملية.'})}});return el};
