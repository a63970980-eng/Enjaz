...
import {mountDemo} from './enjaz-demo-entry.js';
...
if(q.has('demo')){root?.classList.add('is-auth-locked');setAuthState('public');mountDemo()}else if(q.has('auth')){root?.classList.add('is-auth-locked');setAuthState('auth');renderForm()}else if(authClient.token()){recover()}else{window.__ENJAZ_PUBLIC_SHOWN__=true;setAuthState('public')}
