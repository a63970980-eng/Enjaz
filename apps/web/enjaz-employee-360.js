...
async function openEmployee(card){
  ...
  let employee=null,tasks=[],approvals=[],audit=[],integrations=[],tools=[],goals=[],knowledge=[];
  async function load(){
    const main=modal.querySelector('.e360-main'); main.innerHTML='<div class="e360-loading">جارٍ تحميل بيانات الموظف…</div>';
    try{
      const [e,t,a,i,tool,g,k,au]=await Promise.all([
      ...
      employee=e.data||e; tasks=asList(t).filter(x=>x.employee_id===seed.id); approvals=asList(a).filter(x=>x.employee_id===seed.id||x.task?.employee_id===seed.id); integrations=asList(i); tools=asList(tool); goals=asList(g); knowledge=asList(k); audit=asList(au);
  ...
  function renderTab(tab){
    ...
    const skills=asList(employee?.skills||employee?.capabilities), employeeTools=asList(employee?.tools), memory=asList(employee?.memory||employee?.knowledge), permissions=asList(employee?.permissions), activity=audit.filter(x=>x.employee_id===seed.id||x.entity_id===seed.id);
    if(tab==='overview') main.innerHTML=`<div class="e360-kpis">${row('الحالة',employee?.status||'active')}${row('المهام',tasks.length)}${row('المهام المكتملة',tasks.filter(x=>['completed','done','success'].includes(String(x.status).toLowerCase())).length)}${row('الأهداف',goals.length)}${row('المعرفة',knowledge.length)}${row('آخر تحديث',employee?.updated_at?new Date(employee.updated_at).toLocaleString('ar'): '—')}</div><section class="e360-panel"><div class="e360-head"><div><span>IDENTITY</span><h3>ملف الموظف</h3></div></div>${row('الاسم',employee?.name||seed.name)}${row('الدور',employee?.role||seed.role)}${row('الوصف',employee?.description||'—')}</section><section class="e360-panel"><div class="e360-head"><div><span>EXECUTION</span><h3>المهام الحالية</h3></div></div>${list(tasks.slice(0,5),'لا توجد مهام مسندة لهذا الموظف.')}</section>`;
    else if(tab==='tasks') main.innerHTML=`<section class="e360-panel"><div class="e360-head"><div><span>EXECUTION</span><h3>مهام الموظف</h3><p>${tasks.length} مهمة مرتبطة فعليًا بالموظف.</p></div></div>${list(tasks,'لا توجد مهام مسندة لهذا الموظف.')}${approvals.length?`<h4 class="e360-subhead">موافقات مرتبطة</h4>${list(approvals)}`:''}</section>`;
    ...
    else if(tab==='memory') main.innerHTML=`<section class="e360-panel"><div class="e360-head"><div><span>KNOWLEDGE</span><h3>الذاكرة والمعرفة</h3></div></div>${list(knowledge.length?knowledge:memory,'لا توجد ذاكرة أو معرفة مهيأة لهذا الموظف بعد.')}</section>`;
    ...
    else if(tab==='goals') main.innerHTML=`<section class="e360-panel"><div class="e360-head"><div><span>OUTCOMES</span><h3>الأهداف</h3><p>${goals.length} هدف محفوظ فعليًا لهذا الموظف.</p></div></div>${list(goals,'لا توجد أهداف محفوظة لهذا الموظف بعد.')}</section>`;
  ...
  modal.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{modal.querySelectorAll('[data-tab]').forEach(x=>{x.classList.remove('active');x.setAttribute('aria-selected','false');});b.classList.add('active');b.setAttribute('aria-selected','true');renderTab(b.dataset.tab);});
