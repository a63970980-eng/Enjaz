...
async function workspaceSetup(profile){
  ...
  if(current){localStorage.setItem('ENJAZ_WORKSPACE_ID',current.id);sessionStorage.setItem('ENJAZ_WORKSPACES',JSON.stringify(profile.workspaces));return true;}
  ...
  el.querySelector('#workspace-form').onsubmit=async e=>{e.preventDefault();const b=new FormData(e.currentTarget);const btn=e.currentTarget.querySelector('button');btn.disabled=true;btn.textContent='جارٍ التجهيز…';try{const data=await authClient.bootstrap(apiBase,{name:profile.identity.name,organizationName:b.get('organizationName'),workspaceName:b.get('workspaceName')});const ws=data.workspaces?.[0];if(!ws)throw new Error('لم يتم إنشاء مساحة العمل');localStorage.setItem('ENJAZ_WORKSPACE_ID',ws.id);sessionStorage.setItem('ENJAZ_WORKSPACES',JSON.stringify(data.workspaces));location.reload();}catch(err){btn.disabled=false;btn.textContent='إنشاء مساحة العمل ←';el.querySelector('.auth-card').insertAdjacentHTML('beforeend',`<div class="auth-error">${esc(err.message)}</div>`);}};
}
...
if(!authClient.configured){
  ...
  const el=form({signup:q.get('signup')==='1'});
  el.querySelector('#auth-switch').onclick=()=>{q.set('signup',q.get('signup')==='1'?'0':'1');history.replaceState({},'',`${location.pathname}?${q.toString()}`);form({signup:q.get('signup')==='1'});bind();};
  function bind(){const node=document.getElementById('auth-gate');node.querySelector('#auth-switch').onclick=()=>{q.set('signup',q.get('signup')==='1'?'0':'1');history.replaceState({},'',`${location.pathname}?${q.toString()}`);form({signup:q.get('signup')==='1'});bind();};node.querySelector('#auth-form').onsubmit=async e=>{e.preventDefault();const b=new FormData(e.currentTarget),btn=e.currentTarget.querySelector('button');btn.disabled=true;btn.textContent='جارٍ الاتصال…';try{if(q.get('signup')==='1'){await authClient.signUp(b.get('email'),b.get('password'),b.get('name'));}else await authClient.signIn(b.get('email'),b.get('password'));location.reload();}catch(err){btn.disabled=false;btn.textContent=q.get('signup')==='1'?'إنشاء الحساب':'تسجيل الدخول';form({signup:q.get('signup')==='1',error:err.message});bind();}};}
  bind();
}else{
  authClient.me(apiBase).then(profile=>{if(!profile){location.reload();return;}if(profile.workspaces?.[0]){localStorage.setItem('ENJAZ_WORKSPACE_ID',q.get('workspaceId')||localStorage.getItem('ENJAZ_WORKSPACE_ID')||profile.workspaces[0].id);sessionStorage.setItem('ENJAZ_USER_PROFILE',JSON.stringify(profile));}else workspaceSetup(profile);}).catch(err=>mount(`<div class="auth-shell"><div class="auth-card"><div class="auth-copy"><div class="eyebrow">SESSION ERROR</div><h1>تعذر تحميل مساحة العمل</h1><p>${esc(err.message)}</p><button class="primary" onclick="location.reload()">إعادة المحاولة</button></div></div></div>`));
}
