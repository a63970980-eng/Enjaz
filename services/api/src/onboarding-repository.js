...
export async function getUserByAuthId(authUserId){
  const r=await query('select id,auth_user_id,organization_id,email,name,role,created_at from users where auth_user_id=$1 limit 1',[authUserId]);
...
export async function bootstrapWorkspace({authUserId,email,name,organizationName,workspaceName}){
  return withTransaction(async client=>{
    ...
    await client.query('insert into users(id,auth_user_id,organization_id,email,name,role) values($1,$2,$3,$4,$5,$6)',[userId,authUserId,orgId,email,name||email.split('@')[0], 'owner']);
    ...
    return {user:{id:userId,auth_user_id:authUserId,organization_id:orgId,email,name:name||email.split('@')[0],role:'owner'},workspaces:[workspace],created:true};
  });
}
