...
registerTool({name:'task.comment',description:'Add a persistent work note to the current task for the company record.',risk:'low',execute:async({input,context})=>{
 const workspaceId=context.workspaceId,taskId=context.taskId,employeeId=employeeIdFromContext(context);
 ...
 const id=randomUUID();const row=(await query('insert into task_comments(id,workspace_id,task_id,employee_id,body) values($1,$2,$3,$4,$5) returning id,body,created_at',[id,workspaceId,taskId,employeeId,body])).rows[0];
...
registerTool({name:'task.handoff',description:'Hand off the current task to another digital employee in the same workspace.',risk:'low',execute:async({input,context})=>{
 const workspaceId=context.workspaceId,taskId=context.taskId,fromEmployeeId=employeeIdFromContext(context),toEmployeeId=String(input?.toEmployeeId||input?.employeeId||'').trim();
 ...
 const target=(await query("select id,name,role from ai_employees where id=$1 and workspace_id=$2 and status='active'",[toEmployeeId,workspaceId])).rows[0];if(!target)throw new Error('Target employee is not active in this workspace');
 ...
 const childTitle=String(input?.title||`متابعة: ${reason}`).trim();
 const childObjective=String(input?.objective||input?.goal||reason).trim();
 await query('begin');
 try{
  await query(`insert into tasks(id,workspace_id,employee_id,title,objective,priority,parent_task_id,assigned_by_employee_id) values($1,$2,$3,$4,$5,$6,$7,$8)`,[childTaskId,workspaceId,toEmployeeId,childTitle,childObjective,Number(input?.priority)||5,taskId,fromEmployeeId]);
  const handoff=(await query(`insert into task_delegations(id,workspace_id,parent_task_id,child_task_id,from_employee_id,to_employee_id,reason,status,context) values($1,$2,$3,$4,$5,$6,$7,'accepted',$8::jsonb) returning id,parent_task_id,child_task_id,to_employee_id,status,created_at`,[handoffId,workspaceId,taskId,childTaskId,fromEmployeeId,toEmployeeId,reason,JSON.stringify(payload)])).rows[0];
  await query(`insert into employee_handoffs(id,workspace_id,from_employee_id,to_employee_id,task_id,reason,payload,status) values($1,$2,$3,$4,$5,$6,$7::jsonb,'pending')`,[randomUUID(),workspaceId,fromEmployeeId,toEmployeeId,taskId,reason,JSON.stringify({childTaskId,...payload})]);
  await query('commit');return {type:'task_handoff',handoff,childTask:{id:childTaskId,employeeId:toEmployeeId,title:childTitle,objective:childObjective}};
 }catch(error){await query('rollback').catch(()=>{});throw error;}
}});
function employeeIdFromContext(context){return context.employeeId||context.employee?.id||null;}
