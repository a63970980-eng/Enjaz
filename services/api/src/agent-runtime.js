...
export async function runEmployeeTask({workspaceId,taskId,employeeId,action='data.analyze',input={}}){
 ...
 if(needsApproval(action)){const approval=(await query(`insert into approvals (id,workspace_id,task_id,action,reason,payload) values ($1,$2,$3,$4,$5,$6::jsonb) returning *`,[randomUUID(),workspaceId,taskId,action,'Human approval is required before this action can execute.',JSON.stringify({input})])).rows[0];return {status:'awaiting_approval',approval};}
 const output=await executeTool({employee,name:action,input});
