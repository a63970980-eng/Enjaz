...
async function dispatch(job){
 ...
 if(job.job_type==='employee.step'){
  ...
  if(p.graphId&&s.id){const r=await query("select status,output from execution_steps where graph_id=$1 and step_key=$2",[p.graphId,s.id]);const current=r.rows[0];if(current?.status==='succeeded')return {status:'already_completed',output:current.output};if(current?.status==='failed')throw new Error(`Execution step is already failed: ${s.id}`);}
...
export async function processOneJob({workerId=randomUUID()}){await heartbeat({workerId});const job=await claimJob({workerId});if(!job)return null;const attempt=await markAttemptStarted({jobId:job.id,workerId,attempt:job.attempts});try{const result=await dispatch(job);await markAttemptFinished({attemptId:attempt.id,status:'succeeded'});await completeJob({jobId:job.id,workerId,result});return {id:job.id,status:'succeeded',result};}catch(error){await markAttemptFinished({attemptId:attempt.id,status:'failed',error:error.message});await failJob({jobId:job.id,workerId,error});return {id:job.id,status:'failed',error:error.message};}}
