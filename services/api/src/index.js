...
const server=http.createServer(async(req,res)=>{if(req.method==='OPTIONS')return json(res,204,{});const url=new URL(req.url,`http://${req.headers.host}`);try{
 if(req.method==='GET'&&url.pathname==='/health')return json(res,200,{status:'ok',service:'enjaz-api',version:'0.7.0',storage:'postgresql',auth:'supabase',runtime:'enabled',integrations:'enabled'});
 ...
 if(req.method==='POST'&&url.pathname.match(/^\/api\/v1\/tasks\/[^/]+\/run$/)){const id=url.pathname.split('/')[4];const input=await body(req);return json(res,200,{data:await runEmployeeTask({workspaceId,taskId:id,employeeId:input.employeeId,action:input.action||'data.analyze',input:input.input||{}})});}
