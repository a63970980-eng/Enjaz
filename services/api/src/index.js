...
const server=http.createServer(async(req,res)=>{if(req.method==='OPTIONS')return json(res,204,{});const url=new URL(req.url,`http://${req.headers.host}`);try{
 if(req.method==='GET'&&url.pathname==='/health')return json(res,200,{status:'ok',service:'enjaz-api',version:'0.6.0',storage:'postgresql',auth:'supabase',runtime:'enabled'});
 if(req.method==='GET'&&url.pathname==='/api/v1')return json(res,200,{name:'ENJAZ API',version:'0.6.0',storage:'postgresql',auth:'supabase',runtime:'enabled',status:'ready'});
 const user=await authUser(req);if(!user)return json(res,401,{error:'Authentication required'});const workspaceId=url.searchParams.get('workspaceId');if(!workspaceId)return json(res,400,{error:'workspaceId is required'});
 ...
 if(req.method==='POST'&&url.pathname.match(/^\/api\/v1\/tasks\/[^/]+\/run$/)){const id=url.pathname.split('/')[4];const input=await body(req);return json(res,200,{data:await runEmployeeTask({workspaceId,taskId:id,employeeId:input.employeeId,action:input.action,input:input.input||{}})});}
 ...
 const match=url.pathname.match(/^\/api\/v1\/approvals\/([^/]+)\/(approve|reject)$/);if(req.method==='POST'&&match)return json(res,200,{data:await decideApproval(match[1],workspaceId,match[2]==='approve'?'approved':'rejected',user.id)});
