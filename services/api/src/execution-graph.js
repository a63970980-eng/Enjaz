...
export async function advanceGraph({graphId,completedStepKey}){
 return withTransaction(async client=>{
  ...
  else {const remaining=steps.filter(s=>!['succeeded','cancelled'].includes(s.status)&&!ready.some(r=>r.id===s.id));if(remaining.length===0&&ready.length===0)await client.query("update execution_graphs set status='succeeded',updated_at=now() where id=$1 and workspace_id=$2",[graphId,graph.workspace_id]);}
