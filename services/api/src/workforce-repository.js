...
export async function getWorkspaceAccess(workspaceId,userId){
  const r=await query('select role from workspace_members where workspace_id=$1 and user_id=$2 limit 1',[workspaceId,userId]);
...
export async function createApproval(input){
  const task=await query('select id from tasks where id=$1 and workspace_id=$2',[input.taskId,input.workspaceId]);
  if(!task.rowCount) throw new Error('Task does not belong to this workspace');
  return (await query(`insert into approvals (id,workspace_id,task_id,action,reason,payload) values ($1,$2,$3,$4,$5,$6::jsonb) returning *`,[randomUUID(),input.workspaceId,input.taskId,input.action,input.reason||'',JSON.stringify(input.payload||{})])).rows[0];
}
