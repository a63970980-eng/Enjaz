...
export function inspectToolCall(call, employee) {
  ...
  const needsApproval = HIGH_RISK.has(call.action);
  return { allowed: true, blocked: needsApproval, requiresApproval: needsApproval, reason: needsApproval ? 'Human approval is required for this action.' : undefined };
}
...
export function executePlan(plan, employee) {
  return plan.steps.map((step, index) => runStep({ ...step, id: `step_${index + 1}` }, employee));
}
